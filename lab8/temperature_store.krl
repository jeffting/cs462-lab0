ruleset temperature_store {
    ...
    rule generate_report {
      ...
      pre {
        corrId = event:attr("correlation_id")
        temperatures = temperatures()
        sender_eci = event:attr("sender_eci")
        sender_host = event:attr("sender_host")
      }
      event:send(
        { "eci": sender_eci, "eid": "subscription",
          "domain": "manager", "type": "report_submitted", "Tx_host": sender_host,
          "attrs": { "correlation_id": corrId, "temperatures": temperatures, "Rx_id": meta:eci }})
    }
}